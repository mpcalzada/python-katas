...
# 
# For example, to type a letter "R" you would press the 7 key three times (as the screen 
# display for the current character cycles through P->Q->R->S->7). A character is "locked in" 
# once the user presses a different key or pauses for a short period of time (thus, no extra 
# button presses are required beyond what is needed for each letter individually). 
# The zero key handles spaces, with one press of the key producing a space and two presses 
# producing a zero.
# 
# In order to send the message "WHERE DO U WANT 2 MEET L8R" a teen would have to actually do 
# 47 button presses. No wonder they abbreviated.
# 
...
# doesn't distinguish between upper/lowercase characters (but you should allow your module to 
...
# letters by hand, try to avoid doing so! (Imagine you work at a phone manufacturer who 
# might be testing out different keyboard layouts, and you want to be able to test new ones rapidly.)
